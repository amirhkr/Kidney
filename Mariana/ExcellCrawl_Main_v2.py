...
for row_KFRE in sheet_KFRE.iter_rows(min_row=3, max_col=sheet_KFRE.max_column, max_row=sheet_KFRE.max_row):
    ...
    for rowPath in PathologyArray:
        ...
        if row_KFRE[0].value.lower() == rowPath['PatientId'].lower():
            # myDict.clear()
            myDict = {
    ...
    date_KFRE_17MonthPast = date_KFRE - dateutil.relativedelta.relativedelta(months=17)
    ...
    date_KFRE_31MonthPast = date_KFRE - dateutil.relativedelta.relativedelta(months=31)
    ...
    #  7 month range
    if (len(GFR_Array) == 0 and len(GFR_iSTAT_Array) == 0) or (len(URINE_Array) == 0):# no variable found so keep searching for +/-7 month span
        for i in range(len(tempArray)):
            if (((tempArray[i]['datePath'] <= date_KFRE_17MonthPast) and (
                    tempArray[i]['datePath'] >= date_KFRE_24MonthPast)) or
                    ((tempArray[i]['datePath'] >= date_KFRE_31MonthPast) and (
                            tempArray[i]['datePath'] <= date_KFRE_24MonthPast))):
                dictTEST = {
                    "pl2_date": tempArray[i]['datePath'],
                    "pl2_name": tempArray[i]['variableName'],
